...
## =============================================================================
## mle regression
fout_mle <- mle_regression(y = ys, X = Xs)
(bhat_mle <- cbind(fout_mle, ESS = NA))

## =============================================================================
## conditional Dirichlet process regression
fout_cdp <- cDPprior::cdp_regress(y = ys, X = Xs, n_samples = n_samples, 
                                  base_dist = 1, 
                                  base_par = diag(apply(Zs, 2, IQR)/2),
...
## conditional Dirichlet process regression
fout_cdp_nrm <- cDPprior::cdp_regress(y = ys, X = Xs, n_samples = n_samples, 
                                      base_dist = 2, 
                                      base_par = diag(apply(Zs, 2, IQR)/2),
                                      prior_par = list(m = rep(prior_mean, p), 
                                                   tau = rep(prior_sd, p)),
...
## gbayes regression
set.seed(772)
fout_gb <- gbayes_regression(y = ys, X = Xs, 
...
fout_bt <- tryCatch({
  betel_regression(
    y = ys, X = Xs, 
    psi0 = as.numeric(coef(lm(ys ~ Xs - 1))), 
